...
function createDateObj(
    ...
    const defaultModifiers: Record<string, (d: Date, m: number, y: number) => boolean> = {
        'rmd-day-today': (d: Date) =>
            adapter.isSame(adapter.date(d), adapter.date(), 'day'),
        'rmd-day-outside': (d: Date, m: number) =>
...
function isSelected(
    ...
    if (selectionMode === 'range') {
        const range = (selectedDates && typeof selectedDates === 'object' && !(selectedDates instanceof Date) && !Array.isArray(selectedDates)) ? (selectedDates as { start?: Date, end?: Date }) : { start: undefined, end: undefined };
