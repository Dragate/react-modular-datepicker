...
describe('Min Max Disabled Recipe', () => {
  const env = setupComponent(<BoundsTestWrapper />);

  test('should render bounds and disabled dates component', () => {
    const day4Btn = Array.from(env.container.querySelectorAll('.rmdp button')).find((b) => b.textContent?.trim() === '4');
    const day10Btn = Array.from(env.container.querySelectorAll('.rmdp button')).find((b) => b.textContent?.trim() === '10');
    const day15Btn = Array.from(env.container.querySelectorAll('.rmdp button')).find((b) => b.textContent?.trim() === '15');
